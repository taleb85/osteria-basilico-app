                ...
                <div key={user.id} className="surface-glass rounded-2xl p-4 shadow-sm border border-white/15 overflow-hidden">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      ...
                      {user.department && (
                        <p className="text-[10px] text-white/50 font-medium uppercase tracking-wider">{user.department}</p>
                      )}
                    </div>
                    <div className="text-right">
                      <div className="text-[10px] font-bold text-white/50 uppercase tracking-tight">{t.stats_total}</div>
            ...
            >
              <div ref={timesheetHeaderScrollRef} className="overflow-x-hidden">
                ...
                >
                  ...
                  <thead>
                    <tr className="border-b border-white/20" style={{ background: 'rgba(30, 55, 130, 0.85)' }}>
                      <th className="sticky left-0 z-10 box-border py-3.5 pl-4 pr-3 text-center text-[11px] font-semibold uppercase tracking-wider text-white border-r border-r-white/20 md:py-2.5 md:pl-3 md:pr-2" style={{ background: 'rgba(30, 55, 130, 0.85)' }}>
                        {t.employee}
                      </th>
                      {weekDays.map((day, dayIdx) => {
                        ...
                        return (
                          <th key={dStr}
                            ...
                            style={{ background: payrollHighlight ? 'rgba(51,102,204,0.35)' : todayDate ? 'rgba(51,102,204,0.25)' : 'rgba(30, 55, 130, 0.85)' }}
                          >
                      ...
                      <th className="box-border border-l border-l-white/20 px-3 py-3.5 text-center text-[11px] font-semibold uppercase tracking-wider text-white md:px-2 md:py-2" style={{ background: 'rgba(30, 55, 130, 0.85)' }}>
                        {t.stats_total}
                      </th>
                    </tr>
                  </thead>
            ...
            >
              ...
              <thead ref={timesheetTheadRef}>
                <tr className="border-b border-white/15">
                  <th className="sticky left-0 z-10 box-border py-2 pl-4 pr-3 text-center text-[11px] font-semibold uppercase tracking-wider text-white/50 border-r border-r-white/15 md:py-1.5 md:pl-3 md:pr-2" style={{ background: 'rgba(30, 55, 130, 0.85)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', boxShadow: 'none' }}>
                    {t.employee}
                  </th>
                  {weekDays.map((day, dayIdx) => {
                    const todayDate = isToday(day);
                ...
                <tfoot>
                  <tr className="bg-[#3366CC]/5 border-t-2 border-[#3366CC]/35">
                    ...
                    <td className="px-3 py-3 text-center bg-white/5 border-l-[3px] border-l-white/30 md:px-2 md:py-2">
                      ...
                      {showFullTimesheetGrid && (
                        <div className="text-xs font-bold text-white md:text-[10px]">
                          {(() => {
                      ...
                      {plannedOnlyTimesheetGrid &&
                        (() => {
                          ...
                          return gf > 0 ? (
                            <div
                              className="text-xs font-bold text-white md:text-[10px]"
                              title={t.ts_kpi_frozen_official}
                            >
                ...
                return (
                  <div className={`grid gap-4 ${gridCols}`}>
                    {kpiItems.map(({ label, val, color }) => (
                      <div key={label}>
                        <p className="text-[10px] text-white/50 uppercase tracking-wide mb-1">{label}</p>
                ...
                {showHoursPreview && (
                  <div className="bg-white/8 rounded-xl p-3 mb-4">
                    ...
                    <div className="grid grid-cols-3 gap-2 text-center">
                      <div>
                        <p className="text-[10px] text-white/50">{t.ts_kpi_planned}</p>
                        <p className="font-bold text-white/80 text-sm">{fmtHM(closingShift.plannedMins)}</p>
                      </div>
                      <div>
                        <p className="text-[10px] text-white/50">{t.ts_kpi_actual}</p>
                        <p className="font-bold text-white/90 text-sm">{fmtHM(previewMins)}</p>
                      </div>
                      <div>
                        <p className="text-[10px] text-white/50">{t.ts_kpi_delta}</p>
