...
export default function MobileProfileHeader({
  ...
  const { currentUser, effectiveLanguage, dataSyncInProgress, isGlobalRefreshing, postRefreshLocked } = useApp();
  ...
  const shellClass = parentProvidesCardShell
    ...
    : `relative rounded-2xl border border-slate-100 dark:border-white/10 bg-white dark:bg-neutral-900 shadow-[0_4px_16px_-4px_rgba(45,90,39,0.14),0_2px_8px_-4px_rgba(15,23,42,0.08)] dark:shadow-[0_4px_16px_-4px_rgba(0,0,0,0.35)] overflow-hidden ${embeddedInAppHeader ? 'mb-0' : 'mb-2'} ${showOnDesktop ? '' : 'md:hidden'} ${compact ? 'p-2' : ''}`;
