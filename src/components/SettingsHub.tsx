...
export default function SettingsHub() {
  ...
  return (
    <div className="flex flex-col md:flex-row w-full min-h-[80vh]">
      {/* Sidebar */}
      <nav className="md:w-64 w-full md:border-r border-slate-200 bg-white flex md:flex-col flex-row overflow-x-auto md:overflow-x-visible">
        {filteredMenu.map((item) => (
          <button
            key={item.key}
            className={`flex items-center gap-2 px-4 py-3 md:w-full md:justify-start justify-center border-b md:border-b-0 md:border-r-0 border-slate-100 text-sm font-semibold transition-colors ${selected === item.key ? 'bg-accent/10 text-accent' : 'hover:bg-slate-50'}`}
      ...
      <div className="flex-1 min-h-0 overflow-y-auto app-horizontal-pad py-4 pb-[max(1.25rem,env(safe-area-inset-bottom,0px))]">
        ...
        {selected === 'advanced' && (
          <div className="surface-glass-sm p-6 rounded-xl mt-6">
            ...
            <p className="text-slate-600 text-sm mb-4">Backup, ripristino, esportazione dati, impostazioni tecniche.</p>
