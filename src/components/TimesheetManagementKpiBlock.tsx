      ...
      >
        ...
        >
          ...
          <div className="min-w-0 flex-1 flex items-center justify-between gap-2">
            <div className="min-w-0">
              <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-500">{t.stats_approved_hours}</p>
              <p className="text-[10px] leading-snug text-slate-500">{t.stats_approved_shifts_count.replace('{n}', String(approvedShifts.length))}</p>
            </div>
            <p className="text-base font-black tabular-nums leading-none text-slate-900 shrink-0">
        ...
        {showEstimatedCostWidget && (
          ...
          >
            ...
            <div className="min-w-0 flex-1 flex items-center justify-between gap-2">
              <div className="min-w-0">
                <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-500">{t.stats_estimated_cost}</p>
              ...
              <p className={`text-base font-black tabular-nums leading-none shrink-0 ${estimatedCostStats.shiftsWithRate === 0 ? 'text-slate-400' : 'text-slate-900'}`}>
          ...
          <div
            className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg ring-1 ring-inset ${
              pendingCount > 0
                ...
                : 'bg-slate-50 ring-slate-300/80'
            }`}
          >
            <AlertCircle
              className={`h-3.5 w-3.5 ${pendingCount > 0 ? 'text-amber-500' : 'text-slate-400'}`}
            />
          </div>
          <div className="min-w-0 flex-1 flex items-center justify-between gap-2">
            <div className="min-w-0">
              <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-500">{t.stats_pending_shifts}</p>
              <p className="text-[10px] leading-snug text-slate-500">{t.stats_confirmed_not_approved}</p>
            </div>
            <p className={`text-base font-black tabular-nums leading-none shrink-0 ${pendingCount > 0 ? 'text-amber-600' : 'text-slate-900'}`}>
              {pendingCount}
            </p>
          </div>
        </motion.div>
                    ...
                    {pendingShifts.map((s) => {
                      ...
                      return (
                        <div key={s.id} className="flex items-center gap-3 px-4 py-2.5 hover:bg-amber-50/80 transition-colors">
                          <div className="w-7 h-7 rounded-full bg-amber-100 flex items-center justify-center flex-shrink-0 ring-1 ring-amber-200/60">
