...
export function openPresenceQrPrintWindow(dataUrl: string, subtitle: string): void {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>QR — FLOW</title>
  <style>
  ...
  <h1 style="font-size:18px;margin-bottom:8px;">FLOW</h1>
  <img src="${dataUrl.replace(/[\\"]/g, '\\$&')}" alt="QR verifica presenza" width="400" height="400"/>
  <p>${safeSubtitle}</p>
